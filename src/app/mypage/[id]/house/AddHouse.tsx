...
import { postImage } from "@/apis/imageAPI";
...
export default function AddHouse() {
  ...
  const submitFunction = async () => {
    ...
    imageList.forEach(async (e)=>{
      const [imageData, imageError] = await postImage({...e, house_id:data.idx});
      if(imageError) console.error(imageError);
      else console.log(imageData);
    });
      ...
      <div className="d-flex justify-content-start flex-column mt-5">
        ...
        <div
          ...
          <div className="row mb-4">
            <h5 className="fw-bold">대표사진 (1장)</h5>
            {imageList.filter((e)=>(e.name=="thumbnail")).map((e)=>(
              <ImageComponent className={"pb-3"} data={imageList} name={"thumbnail"} title={e.title} />
            ))}
            <ImageInputComponent data={imageList} name={"thumbnail"} />
          </div>
