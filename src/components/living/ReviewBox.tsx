...
export function ReviewBox({name, comment, img, url}){
    ...
    return (
        <div 
            className="card"
            style={{marginBottom:"20px", width:"100%", minWidth:"400px", height:"100%", border:"1px solid lightGray", borderRadius:"15px", overflow:"hidden"}}>
            ...
            <div className="container" style={{width:"100%", float:"left"}}>
                {badgeExample.map((badge, index) => (
                    <span key={index} className="badge" style={{width:"auto", padding:"0 5px", margin:"5px", backgroundColor:"lightgray", color:"gray", fontSize:"15px"}}>
                        {badge}
                    </span>
                ))}
            </div>
